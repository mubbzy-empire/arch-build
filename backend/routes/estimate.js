...
router.post('/', async (req, res) => {
  try {
    const { projectId, budget, title, summary, materials, equipment, dimensions } = req.body;

    const estimate = await generateCostEstimate({
      title, summary, materials, equipment, dimensions,
      budget: budget ? Number(budget) : null,
    });

    if (projectId) {
      db.prepare(`
        INSERT INTO estimates (id, project_id, budget, materials_low, materials_high, labor_low, labor_high, timeline_text, grounded, notes)
        VALUES (@id, @project_id, @budget, @materials_low, @materials_high, @labor_low, @labor_high, @timeline_text, @grounded, @notes)
      `).run({
        id: uuidv4(),
